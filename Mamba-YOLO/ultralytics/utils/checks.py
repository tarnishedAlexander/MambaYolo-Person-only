...
def check_amp(model):
    ...
    def amp_allclose(m, im):
        ...
        torch.serialization.add_safe_globals(['ultralytics.nn.tasks.DetectionModel'])  # Allowlist DetectionModel
        a = m(im, device="cpu", verbose=False)[0].boxes.data  # FP32 inference
        with torch.cuda.amp.autocast(True):
            b = m(im, device="cpu", verbose=False)[0].boxes.data  # AMP inference
        del m
